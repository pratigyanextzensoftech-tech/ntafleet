...
import {  state as APINAME} from '../../../api/index'
...
const ViewStateIndex = () => {
  ...
  useEffect(() => {   
   ...
   const handleEdit = async(row) => {
    ...
    console.log(row)
    try {
    const response = await axios.get(`${APINAME}/${row.state_id}`);
  ...
  Swal.fire({
    title: 'Are you sure?',
    text: `Do you really want to delete user "${row.province_name}"?`,
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      // ✅ Call API to delete
      axios.delete(`${APINAME}/${row.state_id}`)
        .then((res) => {
          console.log(res);
          Swal.fire(
            'Deleted!',
            ` "${row.province_name}" has been deleted.`,
            'success'
          );
  ...
  function GetDataTAble() {
     const columns = [
  { data: "state_id", title: "State ID " },
  { data: "province_name", title: "State Name" },
  { data: "province_abbreviation", title: "	Abbreviation" },
  { data: "country_name", title: "Country" },
{
 ...
 const response = await fetch(`${APINAME}?${params.toString()}`);
   ...
   "province_abbreviation":row.province_abbreviation,
