...
const CustomizerProvider = (props) => {
  ...
  const layoutURL = sessionStorage.getItem('layout') || Object.keys(defaultLayoutObj).pop();
  const layoutValue = Object.values(defaultLayoutObj).pop();
  const location = window.location.pathname.split('/').pop();

  useEffect(() => {
    setLayout(layoutValue);
    setSidebarIconType(sessionStorage.getItem('sidebar_icon_type') || ConfigDB.data.settings.sidebar.iconType);
    classes.map((item) => {
      const item1 = Object.keys(item).pop();
      if (location === item1) {
        const val = Object.values(item).pop();
        sessionStorage.setItem('sidebar_layout', val);
  ...
  const addLayout = (layout) => {
    ...
    sessionStorage.setItem('layout_type', layout);
    setLayout(layout);
  };
  ...
  const addMixBackgroundLayout = (mix_background_layout) => {
    ...
    sessionStorage.setItem('mix_background_layout', mix_background_layout);
  ...
  const addColor = (default_color, secondary_color) => {
    ...
    sessionStorage.setItem('default_color', default_color);
    sessionStorage.setItem('secondary_color', secondary_color);
  };
